...
from pnetworks import MLP, ConvNet, ConvNet_eval, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN
...
def get_dataset(dataset, data_path, args):
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=256,
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    elif model == 'ConvNet_eval':
        net = ConvNet_eval(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
    elif model == 'LeNet':
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, text_feature):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    net = nn.DataParallel(net)
    criterion = criterion.to(args.device)

    text_lab = text_feature
    # [10, 512]
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        real_lab = torch.tensor(np.array([np.ones(args.ipc) * i for i in range(args.num_classes)]),
        ...
        if mode == 'train':
            similarity = output @ text_lab.type(torch.float32).T
            # [10, 10]
            similarity2 = (text_lab.type(torch.float32) @ output.T).T
            loss = (criterion(similarity, real_lab) + criterion(similarity2, real_lab)) / 2
        else:
            similarity = output @ text_lab.type(torch.float32).T
            similarity2 = (text_lab.type(torch.float32) @ output.T).T
            loss = (criterion(similarity, lab) + criterion(similarity2, lab)) / 2

        if mode == 'test':
            acc1 = np.sum(np.equal(np.argmax(similarity.cpu().data.numpy(), axis=-1), lab.cpu().data.numpy()))
            acc2 = np.sum(np.equal(np.argmax(similarity2.cpu().data.numpy(), axis=-1), lab.cpu().data.numpy()))
            acc = (acc1 + acc2) / 2
            acc_avg += acc
...
def evaluate_synset(it_eval, net, net_test, images_train, labels_train, testloader, args, text_feature):
    net = net.to(args.device)
    net_test = net_test.to(args.device)
    lab_text = text_feature

    images_train = images_train.to(args.device)     # [10, 3, 32, 32]
    labels_train = labels_train.to(args.device)     # [10, 512]
    ...
    loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug=False,
                                text_feature=lab_text)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s '
          'train loss = %.6f test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train),
                                                 loss_train, acc_test))

    return net, acc_train, acc_test


def epoch_eval(mode, dataloader, net, optimizer, criterion, args, aug, text_feature):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    net = nn.DataParallel(net)
    criterion = criterion.to(args.device)
...
def evaluate(it_eval, net, images_train, labels_train, testloader, args, text_feature):
    net = net.to(args.device)                       # ConvNet_eval
    lab_text = text_feature                         # [10, 512]

    images_train = images_train.to(args.device)     # [10 * ipc, 3, 32, 32]
    labels_train = labels_train.to(args.device)     # [10 * ipc]
    ...
    start = time.time()
    for ep in range(Epoch+1):
        loss_train, acc_train = epoch_eval('train', trainloader, net, optimizer, criterion, args, aug=True,
                                           text_feature=lab_text)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
    ...
    time_train = time.time() - start
    loss_test, acc_test = epoch_eval('test', testloader, net, optimizer, criterion, args, aug=False,
                                     text_feature=lab_text)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s '
          'train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, Epoch, int(time_train),
