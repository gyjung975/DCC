...
channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader = get_dataset(args.dataset,
                                                                                                     args.data_path)
model_eval_pool = get_eval_pool(eval_mode=args.eval_mode, model=args.model, model_eval=args.model)
# [ConvNet]

accs_all_exps = dict()
for key in model_eval_pool:
    accs_all_exps[key] = []     # {'ConvNet': []}
...
for exp in range(args.num_exp):
    ...
    for it in range(args.Iteration + 1):
        ...
        for model_eval in model_eval_pool:      # [ConvNet]
            print('-------------------------------------------------------------')
            print("Evaluation")
        ...
        ''' visualize and save '''
        save_name = os.path.join(args.save_path, 'vis_%s_%s_%s_%dipc_exp%d_iter%d.png' % (args.method,
                                                                                          args.dataset,
                                                                                          args.model,
                                                                                          args.ipc,
                                                                                          exp,
        ...
        ''' Train synthetic data   -->   Train Generator '''
        ...
        for ol in range(args.outer_loop):
            ...
            for module in net.modules():
                # print(module); print('--------------------'); print(module._get_name())
                if 'BatchNorm' in module._get_name():  # BatchNorm
                    BN_flag = True
            if BN_flag:
                img_real = torch.cat([get_images(c, BNSizePC) for c in range(num_classes)], dim=0)
                net.train()
                # for updating the mu, sigma of BatchNorm
                output_real = net(img_real)
                # get running mu, sigma
                for module in net.modules():
                    if 'BatchNorm' in module._get_name():  # BatchNorm
                        module.eval()  # fix mu and sigma of every BatchNorm layer


            ''' update synthetic data   -->   update Generator '''
            loss = torch.tensor(0.0).to(args.device)

            for c in range(num_classes):
                img_real = get_images(c, args.batch_real)       # c class img batch_real개 get
                # [batch_real, channel, H, W]
        ...
        if it == args.Iteration:    # only record the final results
            ...
            torch.save({'data': data_save, 'accs_all_exps': accs_all_exps, },
                       os.path.join(args.save_path, 'res_%s_%s_%s_%dipc.pt' % (args.method,
                                                                               args.dataset,
                                                                               args.model,
                                                                               args.ipc)))

            torch.save(generator.state_dict(),
                       os.path.join(args.save_path, 'res_%s_%s_%s_%dipc_model.t7' % (args.method,
                                                                                     args.dataset,
                                                                                     args.model,
                                                                                     args.ipc)))
